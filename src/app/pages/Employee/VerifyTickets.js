...
const VerifyTickets = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await TicketService.getTicketByStatus("Verifying");
  ...
  const handleConfirmClick = async (ticketId) => {
    try {
      const response = await TicketService.acceptBookedTicketById(ticketId);
      if (response) {
  ...
  const handleCancelClick = async (ticketId) => {
    try {
      const response = await TicketService.declineBookedTicketById(ticketId);
      if (response) {
        toast.success("Từ chối vé thành công!");
        fetchData();
      }
  ...
  return (
    <Box sx={{}}>
      ...
      <YesNoModal
        title="Xác nhận"
        content="Xác nhận yêu cầu đặt vé này?"
      ...
      <YesNoModal
        title="Từ chối vé"
        content="Từ chối yêu cầu đặt vé này?"
